...
**중단본도 함께 올라갑니다.** `interrupted/` 안의 XML 은 manifest 가 없으므로(중단 시점엔 아직
만들어지지 않음) 계약 없이 전송되고, 서버는 이를 **관측 전용**으로 받습니다 — 열린 포트는 반영되지만
미관측 닫힘 판정 권한은 없습니다. 스캔 이력에서 어떤 것이 중단본인지 구분해 볼 수 있습니다.
