...
- **호스트 상한 없음, 대신 `--watchdog`** — `--host-timeout` 은 어느 강도에서도 걸지 않습니다
  (플래그도 없앴습니다). nmap 은 상한에 걸린 호스트의 포트 표를 **아예 쓰지 않고** 실행 자체는
  `exit="success"` 로 끝내서, 상한 하나가 '살아 있는데 열린 포트가 없다'로 읽혀 그 호스트의 기존
  발견을 전부 닫고 '정상처리'까지 만듭니다 — 되돌리기 가장 어려운 미탐입니다.
  대신 `--watchdog SECONDS` (기본 0=끔)로 nmap 프로세스 하나의 상한을 걸 수 있습니다. 이쪽은
  프로세스를 밖에서 끝내므로 그때까지 끝난 호스트의 관측은 남고(잘린 XML 을 복구합니다), 실행은
  실패로 기록되어 미관측 닫힘 권한을 얻지 못합니다.
  `--script-timeout` 은 그대로 씁니다 — 초과한 스크립트 인스턴스만 죽고 포트 표는 남으므로
  관측을 버리지 않습니다.
