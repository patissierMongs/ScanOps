...
def _use_utf8_stdout() -> None:
    ...
    번들 런처는 chcp 65001 로 UTF-8 을 켜지만, 사람이 직접 실행할 수도 있다. 그때 인코딩할
    수 없는 글자 하나로 **핵심 판정이 traceback 으로 끊기는** 것이 최악이라 대체 문자로
    넘긴다. 본문 자체도 CP949 로 표현 가능한 글자만 쓴다(em dash 대신 하이픈).
    """
