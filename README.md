...
**켜는 방법은 세 표면 모두에 있다.** 웹 단계 스캔은 세부 설정의 [실행 상한] 슬라이더,
레거시/자동 워크플로는 같은 요청 필드(`watchdog_seconds`), 단독 스캐너는 `--watchdog SECONDS`.
기본이 꺼짐인 것과 제어가 아예 없는 것은 다른 문제라, 상한을 없앤 경로마다 대체를 켤 수 있게
두고 계약 테스트로 지킨다.
