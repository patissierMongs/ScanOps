...
### 시간 상한을 쓰지 않는다
`--host-timeout`·`--script-timeout`은 전 구간(웹 단계 엔진·자동 워크플로·단독 스캐너·명령
미리보기)에서 제거했다. 실측에서 소요는 거의 줄지 않은 반면, nmap 은 상한에 걸린 호스트의 포트
...
### 대신 프로세스 워치독 (기본 꺼짐)
상한을 빼면 실행을 끊는 수단이 [중지] 버튼뿐이다. 그래서 `watchdog_seconds` 를 둔다 — nmap
프로세스 하나가 그 시간을 넘기면 밖에서 끝낸다. `--host-timeout` 과 **성질이 정반대**다:

| | `--host-timeout` | 워치독 |
|---|---|---|
| 걸린 호스트의 포트 표 | 버린다 | `-oA` 증분 기록이라 **남는다** |
...
것과 구분해야 상한을 늘릴지 대상을 줄일지 판단할 수 있다).

### 처리량 정책은 모든 단계가 함께 진다
`--max-retries` · `--min-hostgroup 64` · `--max-parallelism 100` · `--defeat-rst-ratelimit` 를
발견·스윕·식별 전 단계에 같은 값으로 싣는다. 한 단계만 빠지면 그 단계가 실행 전체의 꼬리가
되는데, 명령을 나란히 놓고 보기 전에는 그 사실이 드러나지 않는다. 세 경로(웹 옵션 레지스트리·
엔진·단독 스캐너)가 같은 숫자를 쓰는지는 계약 테스트가 검사한다.

두 가지 예외가 있다.
- `--defeat-rst-ratelimit` 는 **SYN 스캔 전용**이다. nmap 은 `-sT`·`-sU`·`-sn` 과 함께 주면
  fatal 로 끝내므로, connect 스캔·UDP·호스트 발견 단계에는 싣지 않는다.
...
이 플래그들은 대상 부하를 **올린다**. 노후 장비가 섞인 대역에는 단독 스캐너의
`--intensity gentle`(`-T3`, RST 율제한 우회 없음, 병렬 10 / 호스트그룹 16 / 재시도 1,
`--max-rate 150`)을 쓴다 — 이제 그 축소가 식별 단계까지 함께 적용된다.
