...
>
> 그 파일은 **스캔 서버의 결과 폴더**(`data/scans/scan_<id>/`)에 있고, `/api/scans` 에는
> 산출물을 내려받는 경로가 없다 — [가져오기]는 브라우저가 있는 PC 의 로컬 파일만 받는다.
> 즉 원격으로 접속한 사용자는 그 폴더를 고를 수 없고, **스캔 서버에 직접 접근할 수 있는
> 관리자만** 꺼낼 수 있다. 화면의 [실행 상한] 설명도 이 접근 경계를 그대로 적는다.
>
