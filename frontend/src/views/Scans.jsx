              ...
              <section className="scan-step">
                ...
                {watchdogMin > 0 && (
                  <div className="muted scan-hint">
                    ...
                    않습니다</b>. 그때까지 끝난 호스트의 관측은 스캔 서버의
                    {" "}<code>data/scans/scan_&lt;스캔번호&gt;/</code> 폴더에 XML 로만 남습니다
                    (닫힘 판정 권한은 얻지 못합니다). 웹에서 내려받는 경로는 없으므로
                    {" "}<b>스캔 서버에 직접 접근할 수 있는 관리자만</b> 그 파일을 꺼낼 수 있습니다.
                    켜기 전에 [중지] 버튼으로 충분한지 먼저 검토하시기 바랍니다.
                    {/* 폴더 규칙은 두 실행 방식에 공통이다(_basename / out_dir 모두
                        scans_dir/scan_<id>). 절대경로까지 보려면 argv 가 필요한데, 그건
                        단계 스캔에만 있다 - 한 번에 실행은 ScanRun.command 에서 -oA 와
                        타깃을 빼고 Nmap argv 이벤트도 남기지 않는다. 그래서 그 안내는
                        단계 스캔 조건으로 묶는다. */}
                    {staged && (
                      <> 절대경로는 [상세] → [실제 실행 명령]의 <code>-oA</code> 인자에서도
                      확인할 수 있습니다(단계 스캔만).</>
                    )}
                  </div>
                )}
