...
SCAN_OPTIONS = [
    ...
    {"key": "syn", "label": "TCP SYN 스캔 (-sS)", "flags": ["-sS"], "group": "스캔 기법", "default": True,
    ...
    {"key": "udp", "label": "UDP 스캔 (-sU)", "flags": ["-sU"], "group": "스캔 기법", "default": True,
    ...
    {"key": "version_all", "label": "버전 탐지·전수 (--version-all)", "flags": ["--version-all"], "group": "탐지", "default": False,
     "note": "느림", "desc": "모든 프로브를 시도해 최대한 정확하게. 그만큼 느리다. 기본은 -sV(intensity 7); 미식별 포트 정밀 재스캔에 켠다."},
    ...
    # ── 결과 표시 ──
    {"key": "open_only", "label": "열린 포트만 (--open)", "flags": ["--open"], "group": "결과 표시", "default": True,
     "desc": "closed/filtered 는 빼고 열린 포트만 출력해 결과를 깔끔하게. '현재 열린 포트' 추적 목표에 부합."},
    {"key": "reason", "label": "판단 근거 (--reason)", "flags": ["--reason"], "group": "결과 표시", "default": True,
...
NSE_SCRIPTS = [
    ...
    {"key": "oracle-tns-version", "group": "DB", "nmap_default": False, "desc": "Oracle TNS 리스너 버전 ⚠ DB 장애 위험(티베로 등 호환DB 다운 사례) — 기본 제외"},
    {"key": "ms-sql-info", "group": "DB", "nmap_default": False, "desc": "MS-SQL 인스턴스 정보 ⚠ DB 장애/부작용 위험 — 기본 제외"},
