...
# Prerequisite: Python 3.13 or 3.12 (x64) on the server (pre-installed). The wheelhouse ships
# cp313 and cp312 win_amd64 wheels; pure-python deps are version-independent.
$ErrorActionPreference = "Stop"
$root = Split-Path -Parent $PSScriptRoot          # ScanOps/
$backend = Join-Path $root "backend"
$wheelhouse = Join-Path $root "packaging\wheelhouse"

# 지원 런타임. 앞에 있는 것을 먼저 고른다(올인원 번들이 담는 3.13 과 같은 순서).
$supported = @("3.13", "3.12")

Write-Host "ScanOps offline install starting..." -ForegroundColor Cyan

# --- Python (x64) 선택: py 런처(-3.13/-3.12) 우선, 없으면 PATH 의 python ---
function Test-SupportedPy($exe, $launcherArgs) {
  try {
    $info = & $exe @launcherArgs -c "import sys,struct;print('%d.%d' % sys.version_info[:2]);print(struct.calcsize('P')*8)" 2>$null
    if ($LASTEXITCODE -ne 0) { return $null }
    if ($supported -contains $info[0] -and $info[1] -eq "64") {
      return @{ exe = $exe; args = $launcherArgs; version = $info[0] }
    }
  } catch {}
  return $null
}
...
foreach ($v in $supported) {
  if (Get-Command py -ErrorAction SilentlyContinue) { $cand += ,@("py", @("-$v")) }
}
if (Get-Command python -ErrorAction SilentlyContinue) { $cand += ,@("python", @()) }

$pick = $null
foreach ($c in $cand) { $pick = Test-SupportedPy $c[0] $c[1]; if ($pick) { break } }

if (-not $pick) {
  $want = $supported -join " / "
  Write-Host "[ERROR] Python $want (x64) 를 찾지 못했습니다." -ForegroundColor Red
  Write-Host "        wheelhouse 는 cp313/cp312-win_amd64 컴파일 휠만 포함합니다(SQLAlchemy/greenlet/pydantic_core)." -ForegroundColor Yellow
  Write-Host "        해결: (1) Python 3.13 (64-bit) 설치  또는  (2) 다른 버전용 휠을 wheelhouse 에 추가." -ForegroundColor Yellow
  Write-Host "        설치가 아예 불가능한 서버라면 올인원 번들(ScanOps_allinone.zip)을 쓰세요 - Python 이 들어 있습니다." -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
  ...
  Write-Host "        가장 흔한 원인: 파이썬 버전/아키텍처 불일치(휠은 cp313/cp312-win_amd64 전용) 또는 pip 누락." -ForegroundColor Yellow
