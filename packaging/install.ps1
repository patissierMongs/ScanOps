...
# Prerequisite: Python 3.10 / 3.11 / 3.12 / 3.13 (x64) on the server (pre-installed).
...
if (-not (Get-Command python -ErrorAction SilentlyContinue)) {
  Write-Host "[ERROR] Python 3.10-3.13 (x64) is required. Please install it first." -ForegroundColor Red
  exit 1
}
